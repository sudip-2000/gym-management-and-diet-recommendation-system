...
if (strlen($_SESSION['adminid'] == 0)) {
    ...
    if (isset($_POST['submit'])) {
        ...
        // Check if the package already exists for the same category
        $sql = "SELECT id FROM tblpackage WHERE PackageName = :Package AND cate_id = :category";
        ...
        if ($query->rowCount() > 0) {
            $errormsg = "Package already exists in this category. Please choose a different name.";
        } else {
    ...
    if (isset($_POST['bookPackage'])) {
        $userId = $_SESSION['adminid']; // Assume the admin is the current user
        $packageId = $_POST['packageId'];
        
        // Check for duplicate booking
        $sql = "SELECT id FROM tbluser_bookings WHERE user_id = :userId AND package_id = :packageId";
        $query = $dbh->prepare($sql);
        ...
        if ($query->rowCount() > 0) {
            $errormsg = "You have already booked this package.";
        } else {
            // Insert new booking
            $sql = "INSERT INTO tbluser_bookings (user_id, package_id) VALUES (:userId, :packageId)";
            $query = $dbh->prepare($sql);
            $query->bindParam(':userId', $userId, PDO::PARAM_INT);
            $query->bindParam(':packageId', $packageId, PDO::PARAM_INT);
            $query->execute();
                            ...
                            ?>
                                    <tbody>
                                        <tr>
                                            ...
                                            <td>
                                                <a href="add-package.php?del=<?php echo htmlentities($result->id); ?>"><button class="btn btn-danger" type="button">Delete</button></a>
                                            </td>
                                        </tr>
                            <?php
                                    $cnt = $cnt + 1;
